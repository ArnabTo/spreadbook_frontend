...
export default function PurchaseOrderCreatePage() {
  ...
  useEffect(() => {
    if (!error) return;
    if (error.startsWith("purchaseOrder.")) {
      setError(t(error));
    }
  }, [t, error]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchPurchaseOrderOptions();
        if (!active) return;
        setOptions(data);
      } catch (e) {
      } finally {
        ...
        if (editId) {
          setLoadingData(true);
          axiosInstance
            .get(`/api/purchase/orders/${editId}/`)
            .then((res) => {
              if (!active) return;
              const q = res.data;
  ...
  const handleProductChange = useCallback(
    (index, productId) => {
      ...
      if (productUnits.length > 0) {
        const defaultPurchase = productUnits.find((pu) => pu.is_default_purchase);
        const purchase = productUnits.find((pu) => pu.is_purchase_unit);
        const fallback = productUnits[0];
        const chosen = defaultPurchase || purchase || fallback;
        newUnit = String(chosen.unit_id);
      } else if (product.purchase_unit_id) {
        newUnit = String(product.purchase_unit_id);
      ...
      if (matched) {
        newRate = matched.purchase_price;
      } else if (product.default_purchase_rate) {
        newRate = product.default_purchase_rate;
      }
  ...
  const handleUnitChange = useCallback(
    (index, unitId) => {
      ...
      if (product && product.unit_prices) {
        ...
        if (matched) {
          setVal(`products.${index}.rate`, matched.purchase_price);
        }
      }
    },
  ...
  const clearAll = useCallback(() => {
    reset({
      supplier: "", currency: "", date: today,
      currency_rate: "1", narration: "", tax_mode: "TAX_INCLUDED",
      attention: "", location: "", po_ref: "",
  ...
  const validate = useCallback((data) => {
    if (!data.supplier) return t("purchaseOrder.validation.supplierRequired");
    if (!data.currency) return t("purchaseOrder.validation.currencyRequired");
    if (!data.date) return t("purchaseOrder.validation.dateRequired");
    const validRows = (data.products || []).filter((r) => r.product);
    if (validRows.length === 0) return t("purchaseOrder.validation.productRequired");
    for (const r of validRows) {
      if (parseFloat(r.qty || 0) <= 0) return t("purchaseOrder.validation.qtyPositive");
      if (parseFloat(r.rate || 0) < 0) return t("purchaseOrder.validation.rateNonNegative");
      if (parseFloat(r.discount_amount || 0) < 0) return t("purchaseOrder.validation.discountNonNegative");
      if (parseFloat(r.tax_percent || 0) < 0) return t("purchaseOrder.validation.taxNonNegative");
    }
    return "";
  }, [t]);
  ...
  const onSave = handleSubmit(async (data) => {
    ...
    try {
      ...
      if (isEdit) {
        await updatePurchaseOrder(editId, formData);
      } else {
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between rounded-xl bg-white px-5 py-3 shadow-sm ring-1 ring-black/5">
        <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span className="font-medium text-gray-800">
            {isEdit ? t("purchaseOrder.editTitle") : t("purchaseOrder.createTitle")}
          </span>
        </div>
      ...
      <div className="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-black/5">
        <div className="bg-[#004080] py-3 text-center">
          <h2 className="text-lg font-bold uppercase tracking-wider text-white">{t("purchaseOrder.title")}</h2>
        </div>

        <div className="grid grid-cols-1 gap-x-6 gap-y-4 p-6 lg:grid-cols-3">
          <div className="flex flex-col gap-3">
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.billNumber")}</label>
              <input
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.supplier")}</label>
              <div className="flex gap-1">
                ...
                <button type="button" className="flex size-9 items-center justify-center rounded-md bg-blue-600 text-white hover:bg-blue-700" title={t("purchaseOrder.addSupplier")}>
                  <Plus className="size-4" />
                </button>
              </div>
            </div>
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.currency")}</label>
              <select
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
                {...register("currency")}
              >
                <option value="">{t("purchaseOrder.selectCurrency")}</option>
                {currencies.map((c) => (
          ...
          <div className="flex flex-col gap-3">
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.date")}</label>
              <input
                type="date"
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
                {...register("date")}
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.poRef")}</label>
              <input
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
                placeholder={t("purchaseOrder.poRefPlaceholder")}
                {...register("po_ref")}
              />
            </div>
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.currencyRate")}</label>
              <input
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.location")}</label>
              <input
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
                placeholder={t("purchaseOrder.locationPlaceholder")}
                {...register("location")}
              />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.narration")}</label>
              <textarea
                rows={3}
                className="w-full resize-y rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
                placeholder={t("purchaseOrder.narrationPlaceholder")}
                {...register("narration")}
              />
            </div>
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.tax")}</label>
              <select
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
                {...register("tax_mode")}
              >
                <option value="TAX_INCLUDED">{t("purchaseOrder.taxIncluded")}</option>
                <option value="TAX_EXCLUDED">{t("purchaseOrder.taxExcluded")}</option>
              </select>
            </div>
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.attention")}</label>
              <textarea
                rows={2}
                className="w-full resize-y rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
                placeholder={t("purchaseOrder.attentionPlaceholder")}
                {...register("attention")}
              />
            </div>
          </div>
            ...
            <table className="w-full min-w-[1200px] text-sm">
              <thead>
                <tr className="bg-[#004080] text-xs font-semibold uppercase text-white">
                  <th className="px-2 py-2 text-center">{t("purchaseOrder.items.siNo")}</th>
                  <th className="px-2 py-2 text-center">{t("purchaseOrder.items.product")}</th>
                  <th className="px-2 py-2 text-center">{t("purchaseOrder.items.unit")}</th>
                  <th className="px-2 py-2 text-center">{t("purchaseOrder.items.qty")}</th>
                  <th className="px-2 py-2 text-center">{t("purchaseOrder.items.rate")}</th>
                  <th className="px-2 py-2 text-center">{t("purchaseOrder.items.discountAmount")}</th>
                  <th className="px-2 py-2 text-center">{t("purchaseOrder.items.productTotal")}</th>
                  <th className="px-2 py-2 text-center">{t("purchaseOrder.items.amount")}</th>
                  <th className="px-2 py-2 text-center">{t("purchaseOrder.items.taxPercent")}</th>
                  <th className="px-2 py-2 text-center">{t("purchaseOrder.items.actions")}</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={field.id} className="border-b">
                      ...
                      <td className="px-2 py-2">
                        ...
                        >
                          <option value="">{t("purchaseOrder.items.selectProduct")}</option>
                          {products.map((p) => (
                      ...
                      <td className="px-2 py-2 text-center">
                        <button
                          ...
                          title={t("purchaseOrder.items.delete")}
                        >
        ...
        <div className="grid grid-cols-1 gap-6 border-t p-6 lg:grid-cols-3">
          <div className="flex flex-col gap-3">
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.total")}</label>
              <input
                className="w-full rounded-lg border border-gray-300 bg-gray-100 px-3 py-2 text-sm outline-none"
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.productDiscountTotal")}</label>
              <input
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.grandTotal")}</label>
              <input
                className="w-full rounded-lg border border-gray-300 bg-gray-100 px-3 py-2 text-sm outline-none"
          ...
          <div className="flex flex-col gap-3">
            ...
            <div>
              <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.cashDiscountTotal")}</label>
              <input
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
          ...
          <div className="flex flex-col gap-3">
            <label className="mb-1 block text-sm font-semibold text-[#004080]">{t("purchaseOrder.attachFile")}</label>
            <div
            ...
            >
              {attachmentFile ? attachmentFile.name : existingAttachmentName || t("purchaseOrder.selectFiles")}
            </div>
            <input
              ref={fileInputRef}
              type="file"
        ...
        <div className="flex items-center justify-center gap-4 border-t px-6 py-4">
          <CreatePermission module="purchase_order">
          <button
            type="button"
            onClick={onSave}
            disabled={saving}
          ...
          >
            <Save className="size-4" /> {t("purchaseOrder.save")}
          </button>
          </CreatePermission>
          <button
            type="button"
          ...
          >
            {t("purchaseOrder.more")} <ChevronDown className="size-4" />
          </button>
          <button
            type="button"
            onClick={clearAll}
          ...
          >
            <Brush className="size-4" /> {t("purchaseOrder.clear")}
          </button>
        </div>
      </div>
