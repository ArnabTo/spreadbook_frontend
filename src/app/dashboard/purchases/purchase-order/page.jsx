...
import {
  swrFetcher, purchaseOrdersKey, deletePurchaseOrder, fetchPurchaseOrderOptions,
} from "@/services/api";
...
export default function PurchaseOrderListPage() {
  ...
  const params = useMemo(() => {
    ...
    return p;
  }, [page, pageSize, billNumber, fromDate, toDate, supplierId, poRef]);

  const key = useMemo(() => purchaseOrdersKey(params), [params]);
  const { data, error, isLoading, mutate } = useSWR(key, swrFetcher, { revalidateOnFocus: false });
  const { data: options } = useSWR("purchase-order-options", () => fetchPurchaseOrderOptions(), { revalidateOnFocus: false });

  const orders = useMemo(() => data?.results || data || [], [data]);
  ...
  const handleDelete = useCallback(async (q) => {
    if (!confirm(t("purchaseOrder.deleteConfirm", { name: q.bill_number }))) return;
    try { await deletePurchaseOrder(q.id); mutate(); } catch { alert(t("purchaseOrder.deleteFailed")); }
  }, [mutate, t]);

  const clearFilters = useCallback(() => {
    setBillNumber(""); setFromDate(""); setToDate("");
    setSupplierId(""); setPoRef("");
    setPage(1);
  }, []);
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between rounded-xl bg-white px-5 py-3 shadow-sm ring-1 ring-black/5">
        <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span>//</span>
          <span className="font-medium text-gray-800">{t("purchaseOrder.title")}</span>
        </div>
      ...
      <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-black/5">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
          <div>
            <input
              ...
              placeholder={t("purchaseOrder.filters.billNumber")}
              value={billNumber}
          ...
          <div className="relative">
            <input
              type="date"
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
              placeholder={t("purchaseOrder.filters.fromDate")}
              value={fromDate}
          ...
          <div className="relative">
            <input
              type="date"
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
              placeholder={t("purchaseOrder.filters.toDate")}
              value={toDate}
          ...
          <div>
            ...
            >
              <option value="">{t("purchaseOrder.filters.supplier")}</option>
              {suppliers.map((c) => (
          ...
          <div>
            <input
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
              placeholder={t("purchaseOrder.filters.poRef")}
              value={poRef}
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button onClick={handleSearch} className="flex size-9 items-center justify-center rounded-md bg-green-600 text-white hover:bg-green-700" title={t("purchaseOrder.filters.search")}>
          ...
          <button onClick={clearFilters} className="flex size-9 items-center justify-center rounded-md bg-red-600 text-white hover:bg-red-700" title={t("purchaseOrder.filters.clear")}>
            <Eraser className="size-4" />
          </button>
          <button className="flex size-9 items-center justify-center rounded-md bg-slate-700 text-white hover:bg-slate-800" title={t("purchaseOrder.filters.exportExcel")}>
            <FileSpreadsheet className="size-4" />
          </button>
          <button className="flex size-9 items-center justify-center rounded-md bg-slate-700 text-white hover:bg-slate-800" title={t("purchaseOrder.filters.exportPdf")}>
      ...
      <div className="flex justify-end gap-2">
        ...
        <button className="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold text-white hover:opacity-90" style={{ backgroundColor: "#808000" }}>
          <Printer className="size-4" />{t("purchaseOrder.actions.printSelected")}
        </button>
      </div>
          ...
          <table className="w-full min-w-[1000px] text-sm">
            <thead>
              <tr className="bg-[#dce3ea] text-xs font-semibold uppercase text-[#004080]">
                ...
                <th className="px-3 py-2 text-left">{t("purchaseOrder.table.billNumber")}</th>
                <th className="px-3 py-2 text-center">{t("purchaseOrder.table.date")}</th>
                <th className="px-3 py-2 text-left">{t("purchaseOrder.table.supplier")}</th>
                <th className="px-3 py-2 text-right">{t("purchaseOrder.table.total")}</th>
                <th className="px-3 py-2 text-right">{t("purchaseOrder.table.tax")}</th>
                <th className="px-3 py-2 text-right">{t("purchaseOrder.table.discount")}</th>
                <th className="px-3 py-2 text-right">{t("purchaseOrder.table.grandTotal")}</th>
                <th className="px-3 py-2 text-center">ACTIONS</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                <tr><td colSpan={9} className="py-16 text-center text-gray-400">{t("purchaseOrder.loading")}</td></tr>
              ) : error ? (
                <tr><td colSpan={9} className="py-16 text-center text-red-400">{t("purchaseOrder.failed")}</td></tr>
              ) : orders.length === 0 ? (
                <tr><td colSpan={9} className="py-16 text-center text-gray-400">{t("purchaseOrder.noData")}</td></tr>
              ) : (
                <>
                  {orders.map((q) => (
                    <tr key={q.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3 text-left text-gray-600">{q.supplier_name || "—"}</td>
                      ...
                      <td className="px-3 py-3 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          <UpdatePermission module="purchase_order">
                            <Link href={`/dashboard/purchases/purchase-order/create?id=${q.id}`} className="flex size-7 items-center justify-center rounded bg-slate-700 text-white hover:bg-slate-800" title={t("purchaseOrder.actions.edit")}>
                              <Pencil className="size-3.5" />
                            </Link>
                          </UpdatePermission>
                          <DeletePermission module="purchase_order">
                            <button onClick={() => handleDelete(q)} className="flex size-7 items-center justify-center rounded bg-slate-700 text-white hover:bg-red-600" title={t("purchaseOrder.actions.delete")}>
