...
export default function SalesOrderRegistryPage() {
  ...
  const params = useMemo(() => {
    ...
    return p;
  }, [page, pageSize, billNumber, fromDate, toDate, customerId, productId, totalAmount]);

  const key = useMemo(() => salesOrderRegistryKey(params), [params]);
  ...
  useEffect(() => {
    async function fetchOptions() {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/api/sales-orders/options/`, {
  ...
  const totals = useMemo(() => {
    return rows.reduce(
      (acc, r) => ({
        ...
        taxTotal: acc.taxTotal + parseFloat(r.tax_total || 0),
        totalAmount: acc.totalAmount + parseFloat(r.total_amount || 0),
      }),
      { quantity: 0, amount: 0, taxTotal: 0, totalAmount: 0 }
    );
  }, [rows]);

  const handleExportExcel = useCallback(async () => {
    try {
      const blob = await exportSalesOrderRegistry(params);
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "sales_order_registry.csv";
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between rounded-xl bg-white px-5 py-3 shadow-sm ring-1 ring-black/5">
        <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span className="font-medium text-gray-800">{t("salesOrderRegistry.title")}</span>
        </div>
      ...
      <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-black/5">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
          <input className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
            placeholder={t("salesOrderRegistry.filters.billNumber")}
          ...
          <select className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
            ...
            <option value="">{t("salesOrderRegistry.filters.customer")}</option>
            {customers.map((c) => (<option key={c.id} value={c.id}>{c.name}</option>))}
          </select>
          <select className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
            value={productId} onChange={(e) => { setProductId(e.target.value); setPage(1); }}>
            <option value="">{t("salesOrderRegistry.filters.product")}</option>
            {products.map((p) => (<option key={p.id} value={p.id}>{p.name}</option>))}
          </select>
          <input className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
            placeholder={t("salesOrderRegistry.filters.totalAmount")}
            value={totalAmount} onChange={(e) => { setTotalAmount(e.target.value); setPage(1); }} />
        </div>
        <div className="mt-4 flex justify-end gap-2">
          <button onClick={handleSearch} className="flex size-9 items-center justify-center rounded-md bg-green-600 text-white hover:bg-green-700" title={t("salesOrderRegistry.filters.search")}>
            <Search className="size-4" />
          </button>
          <button onClick={clearFilters} className="flex size-9 items-center justify-center rounded-md bg-red-600 text-white hover:bg-red-700" title={t("salesOrderRegistry.filters.clear")}>
            <Eraser className="size-4" />
          </button>
          <button onClick={handleExportExcel} className="flex size-9 items-center justify-center rounded-md bg-slate-700 text-white hover:bg-slate-800" title={t("salesOrderRegistry.filters.exportExcel")}>
            <FileSpreadsheet className="size-4" />
          </button>
          <button onClick={handlePrint} className="flex size-9 items-center justify-center rounded-md bg-slate-700 text-white hover:bg-slate-800" title={t("salesOrderRegistry.filters.exportPdf")}>
      ...
      <div className="overflow-hidden rounded-xl shadow-sm ring-1 ring-black/5">
        <div className="rounded-t-xl bg-[#004080] py-3 text-center">
          <h2 className="text-lg font-bold uppercase tracking-wider text-white">{t("salesOrderRegistry.title")}</h2>
        </div>
        <div className="overflow-x-auto bg-white">
          <table className="w-full min-w-[1200px] text-sm">
            <thead>
              <tr className="bg-[#dce3ea] text-xs font-semibold uppercase text-[#004080]">
                <th className="px-3 py-2 text-center">{t("salesOrderRegistry.table.date")}</th>
                <th className="px-3 py-2 text-left">{t("salesOrderRegistry.table.billNumber")}</th>
                <th className="px-3 py-2 text-left">{t("salesOrderRegistry.table.customer")}</th>
                <th className="px-3 py-2 text-left">{t("salesOrderRegistry.table.product")}</th>
                <th className="px-3 py-2 text-center">{t("salesOrderRegistry.table.unit")}</th>
                <th className="px-3 py-2 text-right">{t("salesOrderRegistry.table.quantity")}</th>
                <th className="px-3 py-2 text-right">{t("salesOrderRegistry.table.mrp")}</th>
                <th className="px-3 py-2 text-right">{t("salesOrderRegistry.table.amount")}</th>
                <th className="px-3 py-2 text-right">{t("salesOrderRegistry.table.taxTotal")}</th>
                <th className="px-3 py-2 text-right">{t("salesOrderRegistry.table.totalAmount")}</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                <tr><td colSpan={10} className="py-16 text-center text-gray-400">{t("salesOrderRegistry.loading")}</td></tr>
              ) : error ? (
                <tr><td colSpan={10} className="py-16 text-center text-red-400">{t("salesOrderRegistry.failed")}</td></tr>
              ) : rows.length === 0 ? (
                <tr><td colSpan={10} className="py-16 text-center text-gray-400">{t("salesOrderRegistry.noData")}</td></tr>
              ) : (
                rows.map((row) => (
        ...
        {rows.length > 0 && (
          <div className="border-t bg-gray-50 px-4 py-2">
            <div className="flex justify-end gap-8 text-sm font-semibold text-gray-800">
              <span className="w-24 text-right">{t("salesOrderRegistry.total")}</span>
              <span className="w-20 text-right">{fmt(totals.quantity)}</span>
              <span className="w-20 text-right">{fmt(totals.amount)}</span>
              <span className="w-20 text-right">{fmt(totals.taxTotal)}</span>
              <span className="w-24 text-right">{fmt(totals.totalAmount)}</span>
            </div>
          </div>
        )}
