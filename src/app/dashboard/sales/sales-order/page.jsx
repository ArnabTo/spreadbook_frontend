...
import {
  swrFetcher, salesOrdersKey, deleteSalesOrder, fetchSalesOrderOptions,
} from "@/services/api";
...
export default function SalesOrderListPage() {
  ...
  const key = useMemo(() => salesOrdersKey(params), [params]);
  const { data, error, isLoading, mutate } = useSWR(key, swrFetcher, { revalidateOnFocus: false });
  const { data: options } = useSWR("sales-order-options", () => fetchSalesOrderOptions(), { revalidateOnFocus: false });
  ...
  const handleDelete = useCallback(async (q) => {
    if (!confirm(t("salesOrder.deleteConfirm", { name: q.bill_number }))) return;
    try { await deleteSalesOrder(q.id); mutate(); } catch { alert(t("salesOrder.deleteFailed")); }
  }, [mutate, t]);
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between rounded-xl bg-white px-5 py-3 shadow-sm ring-1 ring-black/5">
        <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span>//</span>
          <span className="font-medium text-gray-800">{t("salesOrder.title")}</span>
        </div>
      ...
      <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-black/5">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
          <div>
            <input
              ...
              placeholder={t("salesOrder.filters.billNumber")}
              value={billNumber}
          ...
          <div className="relative">
            <input
              type="date"
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
              placeholder={t("salesOrder.filters.fromDate")}
              value={fromDate}
          ...
          <div className="relative">
            <input
              type="date"
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
              placeholder={t("salesOrder.filters.toDate")}
              value={toDate}
          ...
          <div>
            ...
            >
              <option value="">{t("salesOrder.filters.customer")}</option>
              {customers.map((c) => (
          ...
          <div>
            <input
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand"
              placeholder={t("salesOrder.filters.poRef")}
              value={poRef}
          ...
          <div>
            ...
            >
              <option value="">{t("salesOrder.filters.salesPerson")}</option>
              {users.map((u) => (
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button onClick={handleSearch} className="flex size-9 items-center justify-center rounded-md bg-green-600 text-white hover:bg-green-700" title={t("salesOrder.filters.search")}>
          ...
          <button onClick={clearFilters} className="flex size-9 items-center justify-center rounded-md bg-red-600 text-white hover:bg-red-700" title={t("salesOrder.filters.clear")}>
            <Eraser className="size-4" />
          </button>
          <button className="flex size-9 items-center justify-center rounded-md bg-slate-700 text-white hover:bg-slate-800" title={t("salesOrder.filters.exportExcel")}>
            <FileSpreadsheet className="size-4" />
          </button>
          <button className="flex size-9 items-center justify-center rounded-md bg-slate-700 text-white hover:bg-slate-800" title={t("salesOrder.filters.exportPdf")}>
      ...
      <div className="flex justify-end gap-2">
        ...
        <button className="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold text-white hover:opacity-90" style={{ backgroundColor: "#808000" }}>
          <Printer className="size-4" />{t("salesOrder.actions.printSelected")}
        </button>
      </div>
          ...
          <table className="w-full min-w-[1000px] text-sm">
            <thead>
              <tr className="bg-[#dce3ea] text-xs font-semibold uppercase text-[#004080]">
                ...
                <th className="px-3 py-2 text-left">{t("salesOrder.table.billNumber")}</th>
                <th className="px-3 py-2 text-center">{t("salesOrder.table.date")}</th>
                <th className="px-3 py-2 text-left">{t("salesOrder.table.customer")}</th>
                <th className="px-3 py-2 text-right">{t("salesOrder.table.total")}</th>
                <th className="px-3 py-2 text-right">{t("salesOrder.table.tax")}</th>
                <th className="px-3 py-2 text-right">{t("salesOrder.table.discount")}</th>
                <th className="px-3 py-2 text-right">{t("salesOrder.table.grandTotal")}</th>
                <th className="px-3 py-2 text-center">ACTIONS</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                <tr><td colSpan={9} className="py-16 text-center text-gray-400">{t("salesOrder.loading")}</td></tr>
              ) : error ? (
                <tr><td colSpan={9} className="py-16 text-center text-red-400">{t("salesOrder.failed")}</td></tr>
              ) : orders.length === 0 ? (
                <tr><td colSpan={9} className="py-16 text-center text-gray-400">{t("salesOrder.noData")}</td></tr>
              ) : (
                <>
                  {orders.map((q) => (
                      ...
                      <td className="px-3 py-3 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          <UpdatePermission module="sales_order">
                            <Link href={`/dashboard/sales/sales-order/create?id=${q.id}`} className="flex size-7 items-center justify-center rounded bg-slate-700 text-white hover:bg-slate-800" title={t("salesOrder.actions.edit")}>
                              <Pencil className="size-3.5" />
                            </Link>
                          </UpdatePermission>
                          <DeletePermission module="sales_order">
                            <button onClick={() => handleDelete(q)} className="flex size-7 items-center justify-center rounded bg-slate-700 text-white hover:bg-red-600" title={t("salesOrder.actions.delete")}>
