...
export default function CategoryPage() {
    ...
    const params = useMemo(() => {
        ...
        if (descriptionFilter) p.description = descriptionFilter;
        if (isChildFilter !== "") p.is_child = isChildFilter;
        return p;
    }, [page, pageSize, search, descriptionFilter, isChildFilter]);

    const key = useMemo(() => categoriesKey(params), [params]);

    const { data, error, isLoading, mutate } = useSWR(key, swrFetcher, {
        revalidateOnFocus: false,
    });

    const categories = useMemo(() => data?.results || data || [], [data]);
                    ...
                    <table className="w-full min-w-[900px] text-sm">
                        <thead>
                            <tr className="bg-[#dce3ea] text-xs font-semibold uppercase text-[#004080]">
                                ...
                                <th className="px-3 py-2 text-center">PARENT CATEGORY ID</th>
                                <th className="px-3 py-2 text-center">CREATED ON UTC</th>
                ...
                <div className="flex items-center justify-between border-t bg-white px-4 py-3">
                    <div className="flex items-center gap-1.5">
                        ...
                        <div className="ml-3 flex items-center gap-1.5">
                            ...
                            <span className="text-xs font-medium text-[#004080]">
                                items per page
                            </span>
                        </div>
                    </div>
