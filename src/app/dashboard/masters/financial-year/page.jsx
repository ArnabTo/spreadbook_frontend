...
export default function FinancialYearPage() {
  ...
  const key = useMemo(() => financialYearsKey(params), [params]);
  const { data, error, isLoading, mutate } = useSWR(key, swrFetcher, { revalidateOnFocus: false });
  const years = useMemo(() => data?.results || data || [], [data]);
  ...
  const handleDelete = useCallback(async (fy) => {
    if (!confirm(t("financialYear.deleteConfirm", { name: fy.name }))) return;
    try { await deleteFinancialYear(fy.id); mutate(); } catch { alert("Failed to delete."); }
  }, [mutate, t]);
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between rounded-xl bg-white px-5 py-3 shadow-sm ring-1 ring-black/5">
        <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span className="font-medium text-gray-800">{t("financialYear.title")}</span>
        </div>
      ...
      <div className="overflow-hidden rounded-xl shadow-sm ring-1 ring-black/5">
        <div className="rounded-t-xl bg-[#004080] py-3 text-center">
          <h2 className="text-lg font-bold uppercase tracking-wider text-white">{t("financialYear.title")}</h2>
        </div>
          ...
          <table className="w-full min-w-[800px] text-sm">
            <thead>
              <tr className="bg-[#dce3ea] text-xs font-semibold uppercase text-[#004080]">
                <th className="px-3 py-2 text-left">{t("financialYear.name")}</th>
                <th className="px-3 py-2 text-center">{t("financialYear.fromDate")}</th>
                <th className="px-3 py-2 text-center">{t("financialYear.toDate")}</th>
                <th className="px-3 py-2 text-center">{t("financialYear.closed")}</th>
                <th className="px-3 py-2 text-center">{t("financialYear.createdDate")}</th>
              ...
              <tr className="border-b bg-[#eef1f5]">
                ...
                <td className="px-3 py-1.5" /><td className="px-3 py-1.5" /><td className="px-3 py-1.5" /><td className="px-3 py-1.5" /><td className="px-3 py-1.5" />
              </tr>
            </thead>
            <tbody>
              {isLoading ? <tr><td colSpan={6} className="py-16 text-center text-gray-400">{t("financialYear.loading")}</td></tr>
               : error ? <tr><td colSpan={6} className="py-16 text-center text-red-400">{t("financialYear.failed")}</td></tr>
               : years.length === 0 ? <tr><td colSpan={6} className="py-16 text-center text-gray-400">{t("financialYear.noData")}</td></tr>
               : years.map((fy) => (
                  <tr key={fy.id} className="border-b hover:bg-gray-50">
                    <td className="px-3 py-3 text-left font-medium text-gray-800">{fy.name || "—"}</td>
                    <td className="px-3 py-3 text-center text-gray-600">{formatDate(fy.from_date)}</td>
                    <td className="px-3 py-3 text-center text-gray-600">{formatDate(fy.to_date)}</td>
                    <td className="px-3 py-3 text-center">{fy.closed ? <span className="text-xs font-medium text-red-500">Closed</span> : <span className="text-xs font-medium text-green-700">Open</span>}</td>
