...
export default function ProductServicesPage() {
  ...
  const params = useMemo(() => {
    ...
    return p;
  }, [page, pageSize, nameFilter, codeFilter, categoryFilter]);

  const key = useMemo(() => productServicesKey(params), [params]);
  ...
  const handleDelete = useCallback(
    async (svc) => {
      ...
      try {
        await deleteProductService(svc.id);
        mutate();
      } catch {
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between rounded-xl bg-white px-5 py-3 shadow-sm ring-1 ring-black/5">
        <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span className="font-medium text-gray-800">Product Services</span>
        </div>
      ...
      <div className="flex justify-end">
        ...
        <Link
          href="/dashboard/masters/product-services/create"
          ...
          <table className="w-full min-w-[1100px] text-sm">
            <thead>
              <tr className="bg-[#dce3ea] text-xs font-semibold uppercase text-[#004080]">
                ...
                <th className="px-3 py-2 text-center">TAX APPLICABLE</th>
                <th className="px-3 py-2 text-center">TAX RATE</th>
                <th className="px-3 py-2 text-center">SALES PRICE</th>
                <th className="px-3 py-2 text-center">QUALITY APPLICABLE</th>
                <th className="px-3 py-2 text-center">MIN SALES RATE</th>
                <th className="px-3 py-2 text-center">AVG QTY</th>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={12} className="py-16 text-center text-red-400">
                    Failed to load.
                  </td>
                </tr>
              ) : services.length === 0 ? (
                ...
                services.map((svc) => (
                  <tr key={svc.id} className="border-b transition-colors hover:bg-gray-50">
                    <td className="px-3 py-3 text-left font-medium text-gray-800">
                      {svc.name || "—"}
                    </td>
                    <td className="px-3 py-3 text-center text-gray-600">{svc.code || "—"}</td>
                    ...
                    <td className="px-3 py-3 text-center text-gray-600">
                      {svc.tax_rate != null ? svc.tax_rate : "—"}
                    </td>
                    <td className="px-3 py-3 text-center text-gray-600">
                      {svc.sales_price}
                    </td>
                    <td className="px-3 py-3 text-center">
                      {svc.quality_applicable ? (
                        <span className="font-bold text-green-600">Yes</span>
                      ) : (
                        <span className="text-gray-400">No</span>
                      )}
                    ...
                    <td className="px-3 py-3 text-center text-gray-600">
                      {svc.minimum_sales_rate != null ? svc.minimum_sales_rate : "—"}
                    </td>
                    <td className="px-3 py-3 text-center text-gray-600">
                      {svc.avg_qty != null ? svc.avg_qty : "—"}
                    </td>
                    <td className="px-3 py-3 text-center">
                      {svc.is_active ? (
                        <span className="text-xs font-medium text-green-700">Active</span>
                      ) : (
                        <span className="text-xs font-medium text-red-500">Inactive</span>
                      )}
                    </td>
                    <td className="px-3 py-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <UpdatePermission module="product_service">
                        <Link
                          href={`/dashboard/masters/product-services/create?id=${svc.id}`}
                          className="flex size-7 items-center justify-center rounded bg-slate-700 text-white hover:bg-slate-800"
                        >
                          <Pencil className="size-3.5" />
                        </Link>
                        </UpdatePermission>
                        <DeletePermission module="product_service">
                        <button
