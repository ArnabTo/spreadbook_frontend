...
import {
  Home, ChevronRight, ChevronDown, Plus, Edit3, Lock, X, RefreshCw,
...
export default function WarehousePage() {
  ...
  const key = useMemo(() => warehousesKey(params), [params]);
  const { data, error, isLoading, mutate } = useSWR(key, swrFetcher, { revalidateOnFocus: false });
  const warehouses = useMemo(() => data?.results || data || [], [data]);
  ...
  const handleDelete = useCallback(async (wh) => {
    ...
    try { await deleteWarehouse(wh.id); mutate(); } catch { alert("Failed to delete."); }
  }, [mutate]);
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="overflow-hidden rounded-xl shadow-sm ring-1 ring-black/5">
        <div className="rounded-t-xl bg-[#004080] py-3 text-center">
          <h2 className="text-lg font-bold uppercase tracking-wider text-white">WAREHOUSE</h2>
        </div>
          ...
          <table className="w-full min-w-[700px] text-sm">
            <thead>
              <tr className="bg-[#dce3ea] text-xs font-semibold uppercase text-[#004080]">
                ...
                <th className="px-3 py-2 text-center">CITY</th>
            ...
            <tbody>
              {isLoading ? <tr><td colSpan={7} className="py-16 text-center text-gray-400">Loading...</td></tr>
               : error ? <tr><td colSpan={7} className="py-16 text-center text-red-400">Failed to load.</td></tr>
               : warehouses.length === 0 ? <tr><td colSpan={7} className="py-16 text-center text-gray-400">No warehouses found.</td></tr>
               : warehouses.map((wh) => (
                  <tr key={wh.id} className="border-b hover:bg-gray-50">
                    <td className="px-3 py-3 text-left font-medium text-gray-800">{wh.name || "—"}</td>
                    <td className="px-3 py-3 text-center text-gray-600">{wh.code || "—"}</td>
                    <td className="px-3 py-3 text-center text-gray-600">{wh.country_name || wh.country || "—"}</td>
                    <td className="px-3 py-3 text-center text-gray-600">{wh.state_name || wh.state || "—"}</td>
                    <td className="px-3 py-3 text-center text-gray-600">{wh.city || "—"}</td>
